...
public class Histogram {
    ...
    public void buildHistogram(BaseTransaction transaction, Table table, int attribute) {
        ...
        RecordIterator recordsIterator = table.iterator(transaction);

        if (!recordsIterator.hasNext()) {
            return;
        }

        this.minValue = quantization(recordsIterator.next(), attribute);
        this.maxValue = minValue;

        while(recordsIterator.hasNext()) {
            float v = quantization(recordsIterator.next(), attribute);

            this.minValue = this.minValue > v ? v : this.minValue;
            this.maxValue = this.maxValue < v ? v : this.maxValue;
        ...
        recordsIterator= table.iterator(transaction);

        while(recordsIterator.hasNext()) {
            float v = quantization(recordsIterator.next(), attribute);
            int index = bucketIndex(v);
            buckets[index].increment(v);
        }
    }
    ...
    private float [] allEquality(float qvalue) {
        ...
        for (int i = 0; i < this.buckets.length; i++) {
            if (i == bucket && buckets[i].getCount() > 0) {
                result[i] = (float) 1.0 / buckets[i].getCount();
            } else {
                result[i] = (float) 0d;
            }
        }
    ...
    private float [] allNotEquality(float qvalue) {
        ...
        for (int i = 0; i < this.buckets.length; i++) {
            if (i == bucket && buckets[i].getCount() > 0) {
                result[i] = (float) (1 - (1.0 / buckets[i].getCount()));
            } else {
                result[i] = (float) 1;
            }
        }
