...
public class QueryPlan {
    ...
    public Iterator<Record> executeOptimal() throws DatabaseException, QueryPlanException {
        ...
        set.add(startTableName);

        QueryOperator minOp = minCostSingleAccess(startTableName);

        map.put(set, minOp);

        for (String tbl : this.joinTableNames) {
            set = new HashSet<>();
            set.add(tbl);

            minOp = minCostSingleAccess(tbl);
        ...
        Map<Set, QueryOperator> passMap = map;
    ...
    public QueryOperator minCostSingleAccess(String table) throws DatabaseException,
        ...
        for (int index: getEligibleIndexColumns(table)) {
            ...
            String colName = this.selectColumnNames.get(index);

            QueryOperator newOp = new IndexScanOperator(this.transaction, table, colName,
                    operator,
                    value);
    ...
    private Map<Set, QueryOperator> minCostJoins(Map<Set, QueryOperator> prevMap,
        ...
        for (Set<String> tbls: prevMap.keySet()) {
            ...
            for (int i = 0; i < this.joinTableNames.size(); i++) {
                ...
                String leftJoin[] = getJoinLeftColumnNameByIndex(i);
                String leftTbl = leftJoin[0];
                String leftCol = leftJoin[1];

                String rightJoin[] = getJoinRightColumnNameByIndex(i);
                String rightTbl = rightJoin[0];
                String rightCol = rightJoin[1];
                ...
                Set sideKey = null;
                QueryOperator prevOp = prevMap.get(tbls);
                QueryOperator sideOp;
                QueryOperator rtnOp;

                if (tbls.contains(leftTbl) && !tbls.contains(rightTbl)) {
                    Set<Set> keys = pass1Map.keySet();
                    for (Set key: keys) {
                        if (key.contains(rightTbl)) {
                            sideKey = key;
                            break;
                        }
                    }
                    assert sideKey != null;
                    sideOp = pass1Map.get(sideKey);
                    rtnOp = minCostJoinType(prevOp, sideOp, leftCol, rightCol);


                } else if (!tbls.contains(leftTbl) && tbls.contains(rightTbl)) {
                    Set<Set> keys = pass1Map.keySet();
                    for (Set key: keys) {
                        if (key.contains(leftTbl)) {
                            sideKey = key;
                            break;
                        }
                    }
                    assert sideKey != null;
                    sideOp = pass1Map.get(sideKey);
                    rtnOp = minCostJoinType(sideOp,prevOp, leftCol, rightCol);
                } else {
                    continue;
                }
                ...
                union.addAll(tbls);
                union.addAll(sideKey);

                int cost = rtnOp.estimateIOCost();

                int prevCost = Integer.MAX_VALUE;
                if (prevMap.containsKey(union)) {
                    prevCost = prevMap.get(union).estimateIOCost();
                }

                int newCost = Integer.MAX_VALUE;
                if (map.containsKey(union)) {
                    newCost = map.get(union).estimateIOCost();
                }
